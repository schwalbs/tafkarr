...
public class ScheduleLookupRequest extends AsyncTask<String, String, ArrayList<String>> {
...
public ScheduleLookupRequest(Context context, RecyclerView resultsList,
                             ScheduleLookupResultsListAdapter resultsListAdapter, ProgressBar progress) {
        mContext = context;
...
private String parseUserSearchHTML(Document doc) {
        ...
        String d = "";
        // course ID
        d += rowData.get(0).firstElementSibling().text();
        d += "\t";
        // prof
        d += rowData.get(3).firstElementSibling().text();
        d += "\t";
        // schedule
